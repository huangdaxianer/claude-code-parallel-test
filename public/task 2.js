...
function renderSidebar() {
    ...
    currentRuns.forEach(run => {
        ...
        tab.innerHTML = `
            ...
            ${isSelected ? '<span style="width:6px; height:6px; background:#000; border-radius:50%;"></span>' : ''}
        `;
...
function renderMainContent() {
    ...
    if (!cleanLog.trim()) {
        ...
        // 使用 Markdown 渲染
        // 只在内容变化时更新
        // 为了防止 markdown 渲染导致的滚动跳动，我们需要更小心的处理
        // 简单处理：如果 raw text 变了，就重新渲染
        if (logDisplayEl.getAttribute('data-raw') !== cleanLog) {
            ...
            // Render Markdown
            // marked is available globally via script tag
            try {
